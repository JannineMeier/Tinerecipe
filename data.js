// data.js
(function () {
  // ---- Polyfill für randomUUID (ältere Browser/iOS) ----
  ...
  const TAG_CATEGORIES = {
    ...
    "Küche": ["Italienisch", "Asiatisch", "Indisch", "Mexikanisch", "Mediterran", "Schweizerisch", "Orientalisch", "Amerikanisch", "International", "Französisch", "Österreichisch", "Fusion"],
    "Aufwand": ["Schnell", "Meal Prep", "< 5 Zutaten", "One-Pot", "Ofen", "Pfanne", "Topf", "Airfryer"]
  };
  const AISLE_ORDER = CATS.slice();
  const STORE_TAGS = ["Migros", "Coop", "Aldi", "Lidl", "Denner", "Party"];

  // ---- Intelligente Einheiten ----
  // Gemüse/Früchte default = Stück; Ausnahmen hier auf g/ml setzen
  const UNIT_HINTS = {
    "kartoffel": { unit: "g", amount: 1000 },
    "kartoffel festkochend": { unit: "g", amount: 1000 },
    "kartoffel mehlig": { unit: "g", amount: 1000 },
    "süsskartoffel": { unit: "g", amount: 800 },
    "karotte": { unit: "g", amount: 500 },
    "zwiebel": { unit: "stk", amount: 2 },
    "zwiebel gelb": { unit: "stk", amount: 2 },
    "zwiebel rot": { unit: "stk", amount: 2 },
    "knoblauch": { unit: "stk", amount: 1 },
    "cherry-tomaten": { unit: "g", amount: 250 },
    "pilze champignons": { unit: "g", amount: 400 },
    "spinat": { unit: "g", amount: 300 },
    "brokkoli": { unit: "stk", amount: 1 },
    "blumenkohl": { unit: "stk", amount: 1 },
    ...
    "mango": { unit: "stk", amount: 1 },
    "granatapfelkerne": { unit: "g", amount: 60 },
    ...
    "mozzarella": { unit: "stk", amount: 1 },
    "feta": { unit: "g", amount: 200 },
    "parmesan": { unit: "g", amount: 100 },
    "creme fraiche": { unit: "g", amount: 200 },
    "käse gerieben": { unit: "g", amount: 150 },
    "eier": { unit: "stk", amount: 10 },
    "rindhack": { unit: "g", amount: 500 },
    "hähnchenbrust": { unit: "g", amount: 400 },
    "rindsgeschnetzeltes": { unit: "g", amount: 400 },
    "lachs frisch": { unit: "g", amount: 300 },
    "thunfisch (dose)": { unit: "stk", amount: 1 },
    "tofu natur": { unit: "g", amount: 400 },
    "reis": { unit: "g", amount: 300 },
    "basmatireis": { unit: "g", amount: 300 },
    "pasta spaghetti": { unit: "g", amount: 300 },
    "pasta penne": { unit: "g", amount: 300 },
    "pasta fusilli": { unit: "g", amount: 300 },
    "haferflocken": { unit: "g", amount: 500 },
    "mehl weiss": { unit: "g", amount: 1000 },
    "tortilla wraps": { unit: "stk", amount: 6 },
    "pizzateig": { unit: "stk", amount: 1 },
    "gnocchi": { unit: "g", amount: 500 },
    "tortellini": { unit: "g", amount: 500 },
    "reispapier": { unit: "stk", amount: 10 },
    "passata": { unit: "ml", amount: 400 },
    "tomaten (dose)": { unit: "stk", amount: 1 },
    "mais (dose)": { unit: "stk", amount: 1 },
    "kokosmilch (dose)": { unit: "stk", amount: 1 },
    ...
    "balsamico": { unit: "ml", amount: 250 },
    "teriyaki sauce": { unit: "ml", amount: 250 },
    "butter chicken sauce": { unit: "stk", amount: 1 },
    "gemüsebouillon": { unit: "stk", amount: 1 },
  ...
  var cart = store.load("cart", []);
  var products = store.load("products", null);
  ...
  function unitHintForName(name) {
    ...
    var alt = n.replace(/e?n?$/, "");
    if (UNIT_HINTS[alt]) return UNIT_HINTS[alt];
    return null;
  }
  var UNIT_OPTIONS = ["", "g", "kg", "ml", "l", "stk"];

  function defaultUnitForCategory(cat) {
    if (cat === "Gemüse" || cat === "Früchte") return "stk";
    if (cat === "Saucen/Öle" || cat === "Getränke") return "ml";
    if (cat === "Fleisch/Fisch" || cat === "Milchprodukte" || cat === "Tiefkühl" || cat === "Gewürze") return "g";
    if (cat === "Getreide/Backwaren" || cat === "Konserven" || cat === "Snacks") return "stk";
    return "stk";
  }
  ...
  function defaultQtyForProduct(name, cat) {
    var hint = unitHintForName(name);
    if (hint) return { unit: hint.unit, amount: hint.amount };
    var u = defaultUnitForCategory(cat || "Sonstiges");
  ...
  function parseQty(str) {
    ...
    var m = s.match(/^(\d+(\.\d+)?)\s*([a-zA-ZäöüÄÖÜ]+)?/);
    if (!m) return { value: null, unit: null, raw: str };
    var val = parseFloat(m[1]);
    var unitRaw = (m[3] || "").toLowerCase();
    var unit = null;
    if (unitRaw === "g" || unitRaw === "gram" || unitRaw === "gramm") unit = "g";
    else if (unitRaw === "stk" || unitRaw === "stück" || unitRaw === "stueck" || unitRaw === "pcs") unit = "stk";
    else if (unitRaw === "kg") unit = "kg";
    else if (unitRaw === "ml") unit = "ml";
    else if (unitRaw === "l" || unitRaw === "lt" || unitRaw === "liter") unit = "l";
    return { value: isNaN(val) ? null : val, unit: unit, raw: str };
  }
  function stepFor(q) {
  ...
  function ensureQtyObject(item) {
    var parsed = parseQty(item.qty || "");
    var unit = parsed.unit;
    var value = parsed.value;
    if (!unit) {
      var def = defaultQtyForProduct(item.name || "", item.cat || "Sonstiges");
      unit = def.unit;
      if (value == null) value = def.amount;
    } else if (value == null) {
      value = defaultAmountForUnit(unit);
    }
    return { qtyStr: formatQty(value, unit), value: value, unit: unit };
  }

  // ---- Produkte seed (einmalig) ----
  if (!products) {
    var veg = [
    ...
    var dairy = [
      ...
      "Käse Gouda", "Käse Emmentaler", "Käse Gruyère", "Mozzarella", "Feta", "Parmesan", "Ricotta", "Mascarpone", "Frischkäse", "Eier", "Creme Fraiche"
    ];
    var meatFish = [
      ...
      "Tofu natur", "Tofu geräuchert", "Tempeh", "Seitan", "Veggie Nuggets", "Rindsteak", "Sucuk"
    ];
    var grain = [
      "Brot", "Brötchen", "Vollkornbrot", "Toast", "Pita", "Tortilla Wraps", "Flammkuchenteig", "Pizzateig",
      "Reis", "Basmatireis", "Jasminreis", "Risotto Reis", "Reisnudeln",
      "Pasta Spaghetti", "Pasta Penne", "Pasta Fusilli", "Lasagneplatten", "Gnocchi", "Tortellini",
      "Couscous", "Bulgur", "Quinoa", "Polenta",
      "Haferflocken", "Cornflakes", "Müesli",
      "Mehl Weiss", "Mehl Vollkorn", "Backpulver", "Hefe", "Paniermehl", "Rösti fixfertig (Beutel)", "Pfannkuchen/Crêpe"
    ];
    var cans = [
      "Tomaten (Dose)", "Passata", "Mais (Dose)", "Kidneybohnen (Dose)", "Kichererbsen (Dose)", "Kokosmilch (Dose)",
      "Linsen (Dose)", "Bohnen weiss (Dose)", "Erbsen (Dose)", "Sugo", "Oliven (Glas)", "Essiggurken"
    ];
    var frozen = [
    ...
    var spices = [
      ...
      "Kreuzkümmel", "Zimt", "Oregano", "Basilikum getrocknet", "Thymian", "Rosmarin", "Knoblauchpulver", "Zwiebelpulver", "Curry Paste rot", "Curry Paste grün", "Gemüsebouillon"
    ];
    var sauces = [
      "Olivenöl", "Rapsöl", "Sonnenblumenöl", "Sesamöl", "Balsamico", "Weissweinessig", "Rotweinessig",
      "Sojasauce", "Worcestersauce", "Sriracha", "Mayonnaise", "Senf", "Ketchup", "BBQ-Sauce", "Pesto grün", "Pesto rot", "Tahini", "Mirin", "Reisessig", "Teriyaki Sauce", "Butter Chicken Sauce", "Honig"
    ];
    var drinks = [
      "Wasser still", "Wasser sprudel", "Apfelsaft", "Orangensaft", "Cola", "Tonic Water", "Mineralwasser",
      "Kaffee Bohnen", "Kaffee gemahlen", "Tee Schwarz", "Tee Grün", "Tee Kräuter", "Haferdrink", "Mandelmilch", "Zitronensaft"
    ];
    var snacks = [
      "Chips", "Nüsse gesalzen", "Erdnüsse", "Mandeln", "Cashews", "Pistazien",
      "Schokolade dunkel", "Schokolade Milch", "Kekse", "Reiswaffeln", "Popcorn", "Proteinriegel", "Pinienkerne", "Tortilla Chips", "Ahornsirup", "Kakaopulver", "Croutons"
    ];

    products = []
      .concat(veg.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Gemüse" }; }))
      .concat(fruit.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Früchte" }; }))
      .concat(dairy.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Milchprodukte" }; }))
      .concat(meatFish.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Fleisch/Fisch" }; }))
      .concat(grain.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Getreide/Backwaren" }; }))
      .concat(cans.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Konserven" }; }))
      .concat(frozen.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Tiefkühl" }; }))
      .concat(spices.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Gewürze" }; }))
      .concat(sauces.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Saucen/Öle" }; }))
      .concat(drinks.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Getränke" }; }))
      .concat(snacks.map(function (n) { return { id: crypto.randomUUID(), name: n, cat: "Snacks" }; }));

    store.save("products", products);
  }

  // ---- Rezepte seed (nur wenn leer) ----
  if (!recipes || recipes.length === 0) {
    recipes = [
      { id: crypto.randomUUID(), title: "Fajita mit Hähnchen", ings: [
      ...
      { id: crypto.randomUUID(), title: "Quesadilla", ings: [
      ...
      { id: crypto.randomUUID(), title: "Bruschetta", ings: [
      ...
      { id: crypto.randomUUID(), title: "Butter Chicken mit Reis & Naan", ings: [
      ...
      { id: crypto.randomUUID(), title: "Reis Bowl", ings: [
      ...
      { id: crypto.randomUUID(), title: "Nudeln mit Lachs und Rahmsauce", ings: [
      ...
      { id: crypto.randomUUID(), title: "Rührei", ings: [
      ...
      { id: crypto.randomUUID(), title: "Ofenkartoffeln (Varianten)", ings: [
      ...
      { id: crypto.randomUUID(), title: "Creamy Feta Gnocchis in Tomatenrahm", ings: [
      ...
      { id: crypto.randomUUID(), title: "Creamy Pasta", ings: [
      ...
      { id: crypto.randomUUID(), title: "Rösti mit Rahmsauce (ohne Pilze)", ings: [
        { name: "Rösti fixfertig (Beutel)", qty: "1 Stk", cat: "Getreide/Backwaren" },
        { name: "Rahm/Sahne", qty: "200 ml", cat: "Milchprodukte" },
        { name: "Zwiebel gelb", qty: "1 Stk", cat: "Gemüse" },
        { name: "Butter", qty: "20 g", cat: "Milchprodukte" }
      ], tags: { Mahlzeit: ["Mittagessen", "Abendessen"], Ernährung: ["Veggie"], Küche: ["Schweizerisch"], Aufwand: ["Pfanne", "Schnell"] } },
      { id: crypto.randomUUID(), title: "Sandwich / Panini", ings: [
      ...
      { id: crypto.randomUUID(), title: "Nudelsalat Caprese", ings: [
      ...
      { id: crypto.randomUUID(), title: "Caesar Salad", ings: [
        ...
        { name: "Zitronensaft", qty: "1 EL", cat: "Getränke" },
        { name: "Knoblauch", qty: "1 Zehe", cat: "Gemüse", optional: true }
      ], tags: { Mahlzeit: ["Mittagessen", "Abendessen"], Ernährung: ["Veggie", "Fleisch"], Küche: ["Amerikanisch"], Aufwand: ["Schnell"] } },
      { id: crypto.randomUUID(), title: "Guacamole-Toast mit Tomaten", ings: [
      ...
      { id: crypto.randomUUID(), title: "Chicken Pad Thai", ings: [
      ...
      { id: crypto.randomUUID(), title: "Smoothie Bowl (Ninja Creamy)", ings: [
      ...
      { id: crypto.randomUUID(), title: "Sticky Salmon Bites mit Reis", ings: [
      ...
      { id: crypto.randomUUID(), title: "Burger mit Salat", ings: [
      ...
      { id: crypto.randomUUID(), title: "Pizza mit Speckstreifen und Mais", ings: [
      ...
      { id: crypto.randomUUID(), title: "Crispy Kartoffelsalat", ings: [
        { name: "Kartoffel festkochend", qty: "1 kg", cat: "Gemüse" },
        { name: "Olivenöl", qty: "2 EL", cat: "Saucen/Öle" },
      ...
      { id: crypto.randomUUID(), title: "Burrito Bowl mit Süsskartoffeln", ings: [
        { name: "Reis", qty: "300 g", cat: "Getreide/Backwaren" },
      ...
      { id: crypto.randomUUID(), title: "Tortellini-Auflauf", ings: [
      ...
      { id: crypto.randomUUID(), title: "Crispy Chickpeas Salat", ings: [
      ...
      { id: crypto.randomUUID(), title: "Crispy Reis Salat", ings: [
        { name: "Reis", qty: "250 g", cat: "Getreide/Backwaren" },
        { name: "Edamame", qty: "150 g", cat: "Gemüse", optional: true },
        { name: "Gurke", qty: "1 Stk", cat: "Gemüse" },
        { name: "Avocado", qty: "1 Stk", cat: "Früchte", optional: true },
      ...
      { id: crypto.randomUUID(), title: "Gebratener Reis mit Ei", ings: [
        { name: "Reis", qty: "300 g", cat: "Getreide/Backwaren" },
      ...
      { id: crypto.randomUUID(), title: "Steak mit Kartoffeln oder Reis", ings: [
      ...
      { id: crypto.randomUUID(), title: "Salat mit Ei", ings: [
        { name: "Salat (Kopf)", qty: "1 Stk", cat: "Gemüse" },
        { name: "Eier", qty: "3 Stk", cat: "Milchprodukte" },
        { name: "Tomate", qty: "1 Stk", cat: "Gemüse" },
      ...
      { id: crypto.randomUUID(), title: "Gözleme mit Spinat", ings: [
      ...
      { id: crypto.randomUUID(), title: "Teriyaki Chicken Summer Rolls", ings: [
      ...
      { id: crypto.randomUUID(), title: "Caprese Salat", ings: [
        { name: "Tomate", qty: "3 Stk", cat: "Gemüse" },
        { name: "Mozzarella", qty: "2 Stk", cat: "Milchprodukte" },
      ...
      { id: crypto.randomUUID(), title: "Mais in Butter angebraten", ings: [
        { name: "Mais (Dose)", qty: "1 Stk", cat: "Konserven" },
        { name: "Butter", qty: "20 g", cat: "Milchprodukte" }
      ], tags: { Mahlzeit: ["Beilage", "Snack"], Ernährung: ["Veggie"], Aufwand: ["Schnell", "< 5 Zutaten", "Pfanne"] } },
      { id: crypto.randomUUID(), title: "Waffeln mit Ahornsirup/Früchten", ings: [
        { name: "Mehl Weiss", qty: "200 g", cat: "Getreide/Backwaren" },
        { name: "Milch", qty: "250 ml", cat: "Milchprodukte" },
        { name: "Eier", qty: "2 Stk", cat: "Milchprodukte" },
      ...
      { id: crypto.randomUUID(), title: "Flammkuchen mit Speck, Käse, Creme Fraiche & Zwiebeln", ings: [
      ...
      { id: crypto.randomUUID(), title: "Kürbissuppe mit Karotten & Zwiebeln", ings: [
      ...
      { id: crypto.randomUUID(), title: "Brokkoli-Cremesuppe", ings: [
      ...
      { id: crypto.randomUUID(), title: "Kokos-Curry-Suppe", ings: [
      ...
      { id: crypto.randomUUID(), title: "Flädlesuppe", ings: [
      ...
      { id: crypto.randomUUID(), title: "Blumenkohlsuppe", ings: [
  ...
  // ---- Core-Zutaten festlegen: alles Andere als Add-on markieren ----
  var CORE_INGS = {
    "Fajita mit Hähnchen": ["Hähnchenbrust", "Paprika", "Tortilla"],
    "Quesadilla": ["Tortilla", "Käse"],
    "Bruschetta": ["Brot", "Tomate", "Olivenöl"],
    "Butter Chicken": ["Hähnchen", "Butter Chicken Sauce", "Reis", "Mehl", "Joghurt", "Backpulver"],
  ...
  function markOptionalsForRecipe(rec) {
    ...
    var key = null;
    for (var k in CORE_INGS) {
      if (title.toLowerCase().indexOf(k.toLowerCase()) !== -1) { key = k; break; }
    }
    if (!key || !Array.isArray(rec.ings)) return rec;
    var normalizedCores = CORE_INGS[key].map(function (s) { return s.toLowerCase(); });
    rec.ings = rec.ings.map(function (ing) {
      var nm = (ing.name || "").toLowerCase();
      var isCore = normalizedCores.some(function (core) { return nm.indexOf(core) !== -1; });
      return {
        name: ing.name,
        qty: ing.qty,
        cat: ing.cat,
        optional: isCore ? false : true
      };
    });
    return rec;
  }

  recipes = (recipes || []).map(markOptionalsForRecipe);
  store.save("recipes", recipes);

  // ---- Export ins globale App-Objekt ----
  window.AppData = {
    CATS: CATS,
    ...
    defaultUnitForCategory: defaultUnitForCategory,
    defaultAmountForUnit: defaultAmountForUnit,
    defaultQtyForProduct: defaultQtyForProduct,
    parseQty: parseQty,
    formatQty: formatQty,
    stepFor: stepFor,
    ensureQtyObject: ensureQtyObject,
    unitHintForName: unitHintForName
  };
})();
