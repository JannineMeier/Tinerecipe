// data.js
(function(){
  // ---- Polyfill für ältere Browser (randomUUID) ----
  ...
  // ---- Intelligente Einheiten & Standardmengen ----
  // Gemüse/Früchte: default = Stück; Ausnahmen hier in g/ml definieren
  const UNIT_HINTS = {
    ...
    // Früchte – Beeren in g, Rest meist Stk
  ...
  // ---- Produkte seed (nur beim ersten Start) ----
  if(!products){
    const veg=[
  ...
  // ---- Rezepte seed (nur wenn noch keine vorhanden) ----
  ...
  // ---- Grundzutaten erkennen: Alles andere als Add-on markieren ----
  (function(){
    // Map Titel → Liste Core-Zutaten (Teilstring-Match, case-insensitive)
    const CORE_INGS = {
    ...
    function markOptionalsForRecipe(rec){
      const key = Object.keys(CORE_INGS).find(k => rec.title.toLowerCase().includes(k.toLowerCase()));
      const cores = key ? CORE_INGS[key] : null;
      if(!Array.isArray(rec.ings)) return rec;
      if(!cores) return rec; // keine Änderung, wenn kein Mapping
